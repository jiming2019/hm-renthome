...
export default class HouseDetail extends Component {
  ...
  componentDidMount() {
    // 获取配置好的路由参数：
    // console.log('路由参数对象：', this.props.location.state.houseCode)
    // console.log(this.props)
    let houseCode = this.props.location.state.houseCode
    // 获取房屋数据
    this.getHouseDetail(houseCode)
    // 获取房源收藏状态
    this.isFavoriteCheck(houseCode)
  }

  isFavoriteCheck = async (houseCode) =>  {
    // 获取房源收藏状态
    let flag = await API.get('/user/favorites/' + houseCode)
    if (flag.status === 200) {
      this.setState({
        isFavorite: flag.body.isFavorite
      })
    }
  }
  ...
  handleFavorite = async () => {
    ...
    // 登录后进行收藏房源
    // 判断当前是否已经收藏，如果已经收藏就取消；否则就收藏
    let { isFavorite } = this.state
    let houseCode = this.props.location.state.houseCode
    if (isFavorite) {
      // 已经收藏了，就删除
      await API.delete('/user/favorites/' + houseCode)
      this.setState({
        isFavorite: false
      })
    } else {
      // 尚未收藏，就收藏
      await API.post('/user/favorites/' + houseCode)
      this.setState({
        isFavorite: true
      })
    }
  ...
  // 获取房屋详细信息
  async getHouseDetail(houseCode) {
    ...
    // )
    const res = await API.get('/houses/' + houseCode)
